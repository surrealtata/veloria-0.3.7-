...
// -----------------------------------------------------------------------------
static const IG_Zone[][IG_ZONE_MAIN] =
{
	...
	{"Bulevar",                     {1370.80,-1577.50,-89.00,1463.90,-1384.90,110.90}},
	...
	{"Granica LS - LV",             {1558.00,596.30,-89.00,1823.00,823.20,110.90}},
	...
	// ---------------------------- MAIN ZONES ---------------------------------
	{"Los Santos",                  {44.60,-2892.90,-242.90,293195597.00,-768.00,900.00}},
	...
	{"Granica LS - SF",             {-1213.90,-2892.90,-242.90,44.60,-768.00,900.00}},
...
task GlobalTimer[1000]()
{
    foreach (new i : Player)
	{
		...
		if(ZonaIgraca(i, zone))
		{
			...
			PlayerTextDrawSetString(i, Ingame_PTD[i][63], string);
		}
		else
		{
			PlayerTextDrawSetString(i, Ingame_PTD[i][63], "Nema signala");
		}
	}
	return 1;
}
...
//
stock ZonaIgraca(playerid, zonename[])
{
	new TmpZone[30];
	GetPlayer2DZone(playerid, TmpZone, sizeof(TmpZone));
	for(new zone = 0; zone < sizeof(IG_Zone); zone++)
	{
		if(strfind(TmpZone, zonename, true) != -1) return 1;
	}
	return 0;
}
...
stock GetPlayer2DZone(playerid, zone[], len)
{
	...
	for(new i = 0; i != sizeof(IG_Zone); i++ )
	{
		if(x >= IG_Zone[i][SAZONE_AREA][0] && x <= IG_Zone[i][SAZONE_AREA][3] && y >= IG_Zone[i][SAZONE_AREA][1] && y <= IG_Zone[i][SAZONE_AREA][4])
		{
			return format(zone, len, IG_Zone[i][IG_ZONE_IMENA], 0);
		}
	}
	return -1;
}
